...
export function AuthControls({ hindi, onAuthenticated, onProfile }: Props) {
  ...
  async function signOut() {
    ...
    applyUser(null); setAccountMenuOpen(false); await onAuthenticated();
  }
  ...
  return <>
    ...
    {open && createPortal(<div className="auth-dialog-backdrop" role="presentation" onMouseDown={() => setOpen(false)}>
      <section role="dialog" aria-modal="true" aria-label={hindi ? "खाता" : "Account"} className="auth-dialog" onMouseDown={(event) => event.stopPropagation()}>
        <div className="flex items-center justify-between gap-4"><div><p className="padhaku-eyebrow">{hindi ? "आपका खाता" : "YOUR ACCOUNT"}</p><h2 className="serif text-2xl font-bold">{mode === "signin" ? (hindi ? "साइन इन करें" : "Sign in") : (hindi ? "खाता बनाएँ" : "Create account")}</h2></div><button type="button" onClick={() => setOpen(false)} aria-label="Close" className="grid size-10 place-items-center rounded-full text-stone-500 hover:bg-stone-900/5"><X className="size-5" /></button></div>
