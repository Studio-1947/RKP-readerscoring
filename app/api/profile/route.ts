...
type Details = { name?: unknown; age?: unknown; phone?: unknown; email?: unknown; place?: unknown; leaderboardOptIn?: unknown; favoriteAuthors?: unknown; favoriteBooks?: unknown };
...
function validDetails(value: Details) {
  ...
  if (full_name.length > 120 || (String(value.age ?? "") && (!Number.isInteger(age) || age < 5 || age > 120)) || (phone && (phone.replace(/\D/g, "").length < 10 || phone.length > 24)) || place.length > 120 || email.length > 254 || !favorite_authors || !favorite_books) return null;
  return { full_name: full_name || null, age: String(value.age ?? "") ? age : null, phone: phone || null, email: email || null, place: place || null, leaderboard_opt_in: value.leaderboardOptIn === true, favorite_authors, favorite_books };
}
...
export async function POST(request: Request) {
  ...
  const profile = body && validDetails(body);
