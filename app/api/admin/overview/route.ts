...
export async function GET(request: Request) {
  ...
  const [profiles, reading, quiz, flags, passages, quizzes, users] = await Promise.all([
  ...
  const results = [profiles, reading, quiz, flags, passages, quizzes, users];
  ...
  const readers = (users.data.users ?? []).map((user) => {
    const profile = profilesById.get(user.id);
    return { id: user.id, full_name: profile?.full_name ?? null, username: profile?.username ?? null, email: profile?.email ?? user.email ?? null, place: profile?.place ?? null, created_at: profile?.created_at ?? user.created_at };
  });
